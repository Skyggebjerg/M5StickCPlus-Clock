...
int bright[4] = {9, 10, 7, 8};

unsigned long myTime;

void setup()
{
  pinMode(39, INPUT_PULLUP); // Side Button
  pinMode(37, INPUT_PULLUP); // Front Button
  M5.begin();
  ...
  // M5.Rtc.SetTime(&TimeStruct); Uncomment if necessary to set the clock
  RTC_DateTypeDef DateStruct;
  DateStruct.WeekDay = 4;
  DateStruct.Month = 1;
  DateStruct.Date = 7;
  DateStruct.Year = 2021;
  // M5.Rtc.SetData(&DateStruct); Uncomment if necessary to set the date
...
void loop()
{
  ...
  M5.Lcd.drawString(String(M5.Axp.GetBatVoltage()) + " V  ", 114, 3, 4);
